...
export default function IncomeExpense() {
      ...
      if (gamesData) {
        ...
        for (const game of gamesData) {
          ...
          const {
            data: salesData,
            error: salesError
          } = await supabase.from('ticket_sales').select('*').eq('game_id', game.id).order('date', {
            ascending: true
          });
  ...
  const generateChartData = (gamesData: GameSummary[]) => {
    ...
    gamesData.forEach(game => {
      game.weeks.forEach(week => {
        ...
        const weekExpenses = game.expenses.filter(expense => expense.date >= week.start_date && expense.date <= week.end_date);
        ...
        weeklyData[weekKey].Profit = week.weekly_sales * (game.organization_percentage / 100) - weeklyData[weekKey].Expenses - weeklyData[weekKey].Donations;
      });
    });
    const sortedTrendData = Object.values(weeklyData).sort((a, b) => new Date(a.date || '').getTime() - new Date(b.date || '').getTime());
  ...
  const calculateSummaryData = () => {
    ...
    if (startDate && endDate) {
      filteredGames = filteredGames.map(game => {
        const filteredSales = game.ticket_sales.filter(sale => sale.date >= startDate && sale.date <= endDate);
        const filteredExpenses = game.expenses.filter(expense => expense.date >= startDate && expense.date <= endDate);
        const filteredWeeks = game.weeks.filter(week => week.start_date >= startDate && week.end_date <= endDate);
        return {
          ...game,
  ...
  const generatePdfReport = async () => {
    try {
      ...
      doc.setFontSize(18);
      doc.text('Queen of Hearts Financial Analytics Report', pageWidth / 2, yPosition, {
        align: 'center'
      });
      yPosition += 15;
      ...
      yPosition += 8;
      const selectedGameName = selectedGame === "all" ? "All Games" : games.find(g => g.id === selectedGame)?.name || "Unknown";
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(11);
      const kpis = [[`Total Revenue: ${formatCurrency(kpiData.totalRevenue)}`, `Total Profit: ${formatCurrency(kpiData.totalProfit)}`], [`Profit Margin: ${formatPercentage(kpiData.profitMargin)}`, `Revenue per Ticket: ${formatCurrency(kpiData.revenuePerTicket)}`], [`Tickets Sold: ${kpiData.totalTicketsSold.toLocaleString()}`, `Growth Rate: ${formatPercentage(kpiData.growthRate)}`]];
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(11);
      const summaryData = [['Total Sales', formatCurrency(summary.totalSales)], ['Total Payouts', formatCurrency(summary.totalPayouts)], ['Total Expenses', formatCurrency(summary.totalExpenses)], ['Total Donations', formatCurrency(summary.totalDonations)], ['Organization Net Profit', formatCurrency(summary.organizationNetProfit)]];
  ...
  return <div className="space-y-6" id="report-container" ref={reportContainerRef}>
      {/* Enhanced Header */}
      <div className="bg-gradient-to-r from-[#1F4E4A] to-[#132E2C] rounded-lg p-6 text-white">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <Button variant="secondary" size="sm" onClick={refreshData} disabled={refreshing}>
              <RefreshCw className={`h-4 w-4 mr-2 ${refreshing ? 'animate-spin' : ''}`} />
              Refresh
            </Button>
            <Button variant="secondary" size="sm" onClick={generatePdfReport}>
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" onClick={() => setQuickDateFilter("all")} className={`text-white border-white/20 hover:bg-white/10 ${!startDate && !endDate ? 'bg-white/20' : ''}`}>
            All Time
          </Button>
          <Button variant="outline" size="sm" onClick={() => setQuickDateFilter("7d")} className="border-white/20 text-teal-50 bg-teal-900 hover:bg-teal-800">
            Last 7 Days
          </Button>
          <Button variant="outline" size="sm" onClick={() => setQuickDateFilter("30d")} className="border-white/20 text-teal-50 bg-teal-900 hover:bg-teal-800">
            Last 30 Days
          </Button>
          <Button variant="outline" size="sm" onClick={() => setQuickDateFilter("90d")} className="border-white/20 text-green-50 bg-teal-900 hover:bg-teal-800">
            Last 90 Days
          </Button>
          <Button variant="outline" size="sm" onClick={() => setQuickDateFilter("1y")} className="border-white/20 text-emerald-50 bg-teal-900 hover:bg-teal-800">
            Last Year
          </Button>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-blue-500 mt-1">
                  {kpiData.growthRate >= 0 ? <span className="flex items-center"><TrendingUp className="h-3 w-3 mr-1" /> +{formatPercentage(kpiData.growthRate)}</span> : <span className="flex items-center"><TrendingDown className="h-3 w-3 mr-1" /> {formatPercentage(kpiData.growthRate)}</span>}
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <Input id="search" placeholder="Search games..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
              </div>
            </div>
            
            <div>
              <Label htmlFor="gameFilter">Game Selection</Label>
              <select id="gameFilter" value={selectedGame} onChange={e => setSelectedGame(e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
            ...
            <div>
              ...
              <select id="reportType" value={reportType} onChange={e => setReportType(e.target.value as "weekly" | "game" | "cumulative")} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
        ...
        <CardContent>
          <div className="h-[500px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              {chartType === "bar" && <BarChart data={chartData} margin={{
              top: 20,
              right: 30,
              left: 20,
              bottom: 20
              ...
              {chartType === "line" && <LineChart data={trendData} margin={{
              top: 20,
              right: 30,
              left: 20,
              bottom: 20
                ...
                fontSize: 12
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Legend />
                  <Line type="monotone" dataKey="Sales" stroke="#A1E96C" strokeWidth={3} dot={{
                r: 4
              }} />
                  <Line type="monotone" dataKey="Payouts" stroke="#1F4E4A" strokeWidth={3} dot={{
                r: 4
              }} />
                  <Line type="monotone" dataKey="Profit" stroke="#132E2C" strokeWidth={3} dot={{
                r: 4
              }} />
                </LineChart>}
              
              {chartType === "pie" && <PieChart margin={{
              top: 20,
              right: 30,
              left: 20,
              bottom: 20
              ...
              {chartType === "area" && <AreaChart data={trendData} margin={{
              top: 20,
              right: 30,
              left: 20,
              bottom: 20
      ...
      {viewMode === "detailed" && <Tabs defaultValue="summary" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-6">
                  {summary.filteredGames.map(game => <div key={game.id} className="bg-gradient-to-r from-gray-50 to-white p-6 rounded-lg border-l-4 border-[#A1E96C]">
                      <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-white p-4 rounded-lg border">
                          ...
                          width: `${Math.min(100, game.organization_net_profit / game.total_sales * 100)}%`
                        }}></div>
                            </div>
                        ...
                        <div className="bg-white p-4 rounded-lg border">
                          ...
                          width: `${Math.min(100, game.total_payouts / game.total_sales * 100)}%`
                        }}></div>
                            </div>
                            <span className="text-sm font-medium">
                        ...
                        <div className="bg-white p-4 rounded-lg border">
                          ...
                          width: `${Math.min(100, (game.total_expenses + game.total_donations) / game.total_sales * 100)}%`
                        }}></div>
                            </div>
                            <span className="text-sm font-medium">
            ...
            <Card>
              ...
              <CardContent>
                <div className="h-[400px] mb-6">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={trendData} margin={{
                  top: 20,
                  right: 30,
                  left: 20,
                  bottom: 20
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Dialog open={addExpenseOpen} onOpenChange={setAddExpenseOpen}>
                    ...
                    <DialogContent>
                      <ExpenseModal open={addExpenseOpen} onOpenChange={setAddExpenseOpen} gameId="" gameName="All Games" />
                    </DialogContent>
                  </Dialog>
                </div>
              </CardHeader>
              <CardContent>
                {summary.filteredGames.map(game => <div key={game.id} className="space-y-6">
                    <h3 className="text-lg font-medium">{game.name}</h3>
                    
                    {/* Ticket Sales Table */}
                    {game.ticket_sales.length > 0 && <div>
                        ...
                        <div className="overflow-auto">
                          <Table>
                            ...
                            <TableBody>
                              {game.ticket_sales.map(sale => <TableRow key={sale.id}>
