  ...
  const addRow = async () => {
    ...
    try {
      ...
      const { data: latestSale, error: latestSaleError } = await supabase
        .from('ticket_sales')
        .select('cumulative_collected, ending_jackpot_total')
      ...
      if (weekEntries >= 6) {  // After adding the 7th entry (index 6)
  ...
  const submitWinner = async () => {
    ...
    try {
      ...
      const { data: configData, error: configError } = await supabase
        .from('configurations')
        .select('card_payouts, penalty_percentage, penalty_to_lodge')
        .limit(1);
        
      if (configError) throw configError;
      
      const config = configData && configData.length > 0 ? configData[0] : { 
        card_payouts: { "Queen of Hearts": "jackpot" },
      ...
      const currentJackpot = latestSale && latestSale.length > 0 
        ? latestSale[0].ending_jackpot_total 
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <input
                  ...
                  type="number"
                  className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                  value={gameForm.lodgePercentage}
                  onChange={(e) => {
            ...
            <div className="grid gap-2">
              ...
              <input
                ...
                onChange={(e) => setGameForm({...gameForm, carryoverJackpot: parseFloat(e.target.value)})}
                min="0"
              />
            </div>
          </div>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <div className="flex h-10 w-full rounded-md border border-input">
                <input
                  ...
                  className="flex-1 bg-background px-3 py-2 text-sm"
                  value={rowForm.date}
                  onChange={(e) => setRowForm({...rowForm, date: e.target.value})}
                />
                <div className="flex items-center pr-3">
                  <CalendarIcon className="h-4 w-4 text-muted-foreground" />
                </div>
            ...
            <div className="grid gap-2">
              ...
              <input
                ...
                onChange={(e) => setRowForm({...rowForm, ticketsSold: parseInt(e.target.value)})}
                min="0"
              />
            </div>
          </div>
