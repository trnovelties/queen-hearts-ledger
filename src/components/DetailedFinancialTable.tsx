...
export function DetailedFinancialTable({ games, formatCurrency }: DetailedFinancialTableProps) {
  ...
  return (
    <div className="space-y-6">
      {games.map(game => {
        console.log(`=== DETAILED TABLE GAME DEBUG: ${game.name} ===`);
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="mt-1">
                      Started: {(() => {
                        console.log('Formatting start_date:', game.start_date);
                        const result = formatDateStringForDisplay(game.start_date);
                        console.log('formatDateStringForDisplay result:', result);
                        return result;
                      })()}
                      {game.end_date && (() => {
                        console.log('Formatting end_date:', game.end_date);
                        const result = formatDateStringForDisplay(game.end_date);
                        console.log('formatDateStringForDisplay end result:', result);
                        return ` | Ended: ${result}`;
                      })()}
                    </CardDescription>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="overflow-x-auto">
                      <Table>
                        ...
                        <TableBody>
                          {game.weeks.map(week => {
                            console.log(`=== WEEK DATE DEBUG: Week ${week.week_number} ===`);
                            console.log('Week start_date raw:', week.start_date);
                            console.log('Week end_date raw:', week.end_date);
                            
                            const startDateFormatted = formatDateStringShort(week.start_date);
                            const endDateFormatted = formatDateStringShort(week.end_date);
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {game.expenses.map(expense => {
                            ...
                            const expenseDateFormatted = formatDateStringForDisplay(expense.date);
                            console.log('formatDateStringForDisplay expense result:', expenseDateFormatted);
