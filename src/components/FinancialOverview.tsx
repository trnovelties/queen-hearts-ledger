        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            <div className="space-y-3 flex-1">
              ...
              <p className={`text-3xl font-bold ${colors.text} font-inter`}>
                {typeof value === 'number' ? formatCurrency(value) : value}
              </p>
              {subtitle && (
                <p className="text-sm text-[#132E2C]/60 font-medium">{subtitle}</p>
              )}
              {trend !== 0 && (
        ...
        <Card className="bg-gradient-to-br from-[#A1E96C]/10 to-[#A1E96C]/5 border-[#A1E96C]/30 border-2">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-4">
              <div className="flex justify-between items-center p-4 bg-white/80 rounded-lg border">
                <div className="flex items-center gap-3">
                  <Users className="h-5 w-5 text-[#1F4E4A]" />
                  <span className="font-semibold text-[#132E2C]">Total Tickets</span>
                </div>
                <span className="font-bold text-[#1F4E4A] text-lg">{summary.totalTicketsSold.toLocaleString()}</span>
              </div>
              
              <div className="flex justify-between items-center p-4 bg-white/80 rounded-lg border">
                <div className="flex items-center gap-3">
                  <DollarSign className="h-5 w-5 text-[#1F4E4A]" />
                  <span className="font-semibold text-[#132E2C]">Gross Revenue</span>
                </div>
                <span className="font-bold text-[#1F4E4A] text-lg">{formatCurrency(summary.totalSales)}</span>
              </div>
              
              <div className="flex justify-between items-center p-4 bg-white/80 rounded-lg border">
                <div className="flex items-center gap-3">
                  <BarChart3 className="h-5 w-5 text-[#1F4E4A]" />
                  <span className="font-semibold text-[#132E2C]">Avg per Game</span>
                </div>
                <span className="font-bold text-[#1F4E4A] text-lg">{formatCurrency(avgGameRevenue)}</span>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center p-4 bg-[#F7F8FC] rounded-lg border">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-semibold text-[#132E2C]">Jackpot Portion</span>
                </div>
                <span className="font-bold text-orange-600 text-lg">{formatCurrency(summary.jackpotTotalPortion)}</span>
              </div>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-4">
              <div className="flex justify-between items-center p-4 bg-white/80 rounded-lg border">
                <div className="flex items-center gap-3">
                  <DollarSign className="h-5 w-5 text-[#1F4E4A]" />
                  <span className="font-semibold text-[#132E2C]">Org. Share</span>
                </div>
                <span className="font-bold text-[#1F4E4A] text-lg">{formatCurrency(summary.organizationTotalPortion)}</span>
              </div>
              
              <div className="flex justify-between items-center p-4 bg-white/80 rounded-lg border">
                <div className="flex items-center gap-3">
                  <HeartHandshake className="h-5 w-5 text-purple-600" />
                  <span className="font-semibold text-[#132E2C]">Donations ({donationRatio.toFixed(1)}%)</span>
                </div>
                <span className="font-bold text-purple-600 text-lg">{formatCurrency(summary.totalDonations)}</span>
              </div>
              
              <div className="flex justify-between items-center p-4 bg-white/80 rounded-lg border">
                <div className="flex items-center gap-3">
                  <Receipt className="h-5 w-5 text-red-600" />
                  <span className="font-semibold text-[#132E2C]">Expenses ({expenseRatio.toFixed(1)}%)</span>
                </div>
                <span className="font-bold text-red-600 text-lg">{formatCurrency(summary.totalExpenses)}</span>
              </div>
              
              <div className="flex justify-between items-center p-4 bg-[#A1E96C]/20 rounded-lg border-2 border-[#A1E96C]/50">
                <div className="flex items-center gap-3">
                  <TrendingUp className="h-5 w-5 text-[#1F4E4A]" />
                  <span className="font-bold text-[#132E2C]">Net Profit</span>
                </div>
                <span className="font-bold text-[#1F4E4A] text-xl">{formatCurrency(summary.organizationNetProfit)}</span>
              </div>
            </div>
          </CardContent>
        </Card>
